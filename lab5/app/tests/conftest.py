...
@pytest.fixture
def visit_log_repository(db_connector):
    return VisitLogRepository(db_connector)
...
def existing_user(db_connector):
    ...
    with db_connector.connect() as conn:
        with conn.cursor() as cursor:
            ...
            cursor.execute("""
                ...
                VALUES (%s, %s, %s, %s, %s)
            """, (unique_username, 'hashed_password', 'Admin', 'User', 1))
    ...
    # Удаляем записи в связанных таблицах, если нужно
    with db_connector.connect() as conn:
        with conn.cursor() as cursor:
            cursor.execute("DELETE FROM visit_logs WHERE user_id = %s", (user_id,))
